...
- **Company data loading** is handled by `company-details.js`:
  - `company-data.js` keeps an in-memory `companyDetailsCache` Map (keyed by company URL, popup-lifetime only) to avoid redundant fetches within the same popup session
...
- **Refresh button (↻)** in the active header: calls `refreshCompanyDetails(item)`, which clears both the in-memory cache entry and the persistent cache entry for that company, resets `data-initialized`, restores original attribute values, and re-runs the fetch pipeline
...
- Entries are keyed by **company id** (`extractCompanyId(companyLink)`, the numeric id from the LinkedIn company URL) — the same key already used for saved leads and the hidden `#company-id` field
- Each entry is `{ companyId, data }`, where `data` is the full company details object (`website`, `location`, `industry`, `size`, `members`, `status`, `ok`)
- List semantics (most-recently-used first, capped at `MAX_CACHED_COMPANIES` = 10 from `src/constants/config.js`) are implemented as pure, unit-tested functions in `src/utils/company-cache.js` (`upsertCompanyCacheEntry`, `findCompanyCacheEntry`, `removeCompanyCacheEntry`); `company-cache-store.js` is a thin `chrome.storage.local` wrapper around them
- `getCompanyData()` in `company-data.js` checks, in order: (1) the in-memory Map (same popup session), (2) the persistent cache by company id, (3) a live LinkedIn fetch. A live fetch that completes successfully is written back into the persistent cache
- The refresh button (↻) removes the persistent entry for that company (via `clearCompanyCache`) in addition to the in-memory one, forcing a live re-fetch on demand

```mermaid
flowchart LR
    Request[getCompanyData] --> MemCache{In-memory Map hit?}
    MemCache -- yes --> Return[Return cached details]
    MemCache -- no --> PersistCache{Persistent cache hit by company id?}
...
src/
 ...
 └── utils/
      ├── company-cache.js             (upsertCompanyCacheEntry, findCompanyCacheEntry, removeCompanyCacheEntry — pure, tested)
...
| `tests/company-cache.test.js` | `src/utils/company-cache.js` | `upsertCompanyCacheEntry` (insert, move-to-front, MRU eviction at max size), `findCompanyCacheEntry`, `removeCompanyCacheEntry` |
| `tests/company-cache-store.test.js` | `src/scripts/store/company-cache-store.js` | `getCachedCompany`, `setCachedCompany` (persists, caps at 10), `removeCachedCompany`; Chrome Storage mocked via `jest.unstable_mockModule` |
