...
- **Public profile page extraction** (`linkedin-pages/lead/lead.js` + `lead-experience.js`): extracts the same personal-data/experience shape as the Sales Navigator version, but without SN's hover-tooltip company data (not available on this page — `extraData` is always empty and gets filled in later by the normal company-page-scrape pipeline) and without the dropdown-menu profile-link lookup (`Link` is simply `window.location.href`, trimmed of the query string, since we're already on the profile page).
  ...
  - The single-position (no `<ul>`) shape sometimes appends the employment type straight onto the company-name `<p>` instead of rendering it as its own sibling, e.g. `"Global Message Services · Full-time"` — `cleanCompanyName()`'s `stripTrailingMetadata()` cuts off everything from that `" · "` separator onward before the existing status-suffix cleanup runs.
...
**Max saved leads (pub/sub):** `src/scripts/store/max-leads-store.js` follows the same `state`/`subscribe`/`notify` pattern as `filter-store.js`, holding the user-configurable max-saved-leads limit (`chrome.storage.sync` key `maxSavedLeads`, default 99, hard cap 999 from `MAX_SAVED_LEADS_LIMIT` in `src/constants/config.js`). `storage-actions.js` subscribes to it to keep the Save button, counter, and progress bar in sync when the limit changes.
...
**Storage actions** (`Save`/`Get`/`Clean`) live in `src/scripts/containers/data/storage-actions.js`. They use `chrome.storage.local` with key `saved_leads` (email is the unique key). The max item count is user-configurable via the "Max saved leads" setting (`src/scripts/containers/settings/max-saved-leads.js`, 1-999, default 99); lowering it below the current saved count prompts for confirmation before deleting the oldest leads. The Get button copies all saved leads to the clipboard in tab-separated format (spreadsheet-friendly) by default, or as a JSON array of lead objects, per the "Export format" setting (`src/scripts/containers/settings/export-format.js`, `chrome.storage.sync` key `leadsExportFormat`, values from `LEADS_EXPORT_FORMATS` in `src/constants/config.js`). The row/object shaping itself (`buildLeadRecord`/`formatLeadsAsTsv`/`formatLeadsAsJson`) is a pure, unit-tested helper in `src/utils/lead-export.js`.
