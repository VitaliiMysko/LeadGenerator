...
- **Name/surname extraction also checks the heading's `aria-label`/`title`**, in case LinkedIn's narrow Sales Navigator panel visually truncates the heading so its `textContent` only reflects the truncated display text; whichever of the three candidates is longest is used
