...
- **Last character of the name could still be missing even with a recognized letter** (e.g. "Miha Kampuš" extracted as "Miha Kampu"): the LinkedIn lead heading can be visually truncated in the narrow Sales Navigator panel, in which case its `textContent` only reflects the truncated display text. Extraction now also checks the heading's `aria-label`/`title` attributes — commonly kept as the full, untruncated name for accessibility — and uses whichever candidate is longest
